...
#include "motion_msgs/msg/path_segment.hpp"
#include <iostream>
#include <string>
...
public:
    TestMoveNode() : Node("test_node"), motion_completed_(false) {
        ...
        // 初始化Action客户端
        action_client_ = rclcpp_action::create_client<motion_msgs::action::MovePath>(
    ...
    void testMove(const std::string& move_type) {
        ...
        switch (type) {
            ...
            default:
                ...
                motion_completed_ = true; // 只有在错误情况下才立即标记完成
                break;
        }
    }
...
private:
    // Action客户端成员变量，确保生命周期与节点一致
    rclcpp_action::Client<motion_msgs::action::MovePath>::SharedPtr action_client_;
    ...
    void sendMotionGoal(const motion_msgs::action::MovePath::Goal& goal_msg, const std::string& motion_type) {
        ...
        if (!action_client_->wait_for_action_server(std::chrono::seconds(10))) {
            ...
            motion_completed_ = true; // 服务器不可用，标记运动完成
            return;
        }
    ...
    void moveToHomePosition() {
        ...
        std::vector<int64_t> target_axes = {5, 4}; // X轴是轴5，Y轴是轴4
...
int main(int argc, char** argv) {
    try {
        ...
        
        {
            ...
            int max_wait_seconds = 30; // 最大等待时间30秒
            int wait_count = 0;
            
            RCLCPP_INFO(node->get_logger(), "等待运动完成...");
            
            while (rclcpp::ok() && !node->isMotionCompleted() && wait_count < max_wait_seconds * 10) {
                rclcpp::spin_some(node);
                rate.sleep();
                wait_count++;
            }
            
            if (node->isMotionCompleted()) {
                RCLCPP_INFO(node->get_logger(), "运动完成");
            } else {
                RCLCPP_WARN(node->get_logger(), "运动超时，强制退出");
            }        
        ...
        RCLCPP_ERROR(rclcpp::get_logger("main"), "捕获到异常: %s", e.what());
        rclcpp::shutdown();
        return 1;
    } catch (...) {
        RCLCPP_ERROR(rclcpp::get_logger("main"), "捕获到未知异常");
        rclcpp::shutdown();
        return 1;
    }
}